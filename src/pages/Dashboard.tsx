...
const EmptyBubbleState = () => {
  return <div className="flex flex-col items-center justify-center py-16 text-center">
      <div className="w-24 h-24 mb-6 relative">
        ...
        <div className="animate-bubble-float absolute w-12 h-12 rounded-full bg-[#F9C80E] opacity-80 right-0 top-4" style={{
        animationDelay: '0.5s'
      }}></div>
        <div className="animate-bubble-float absolute w-8 h-8 rounded-full bg-[#F9C80E] opacity-70 left-2 bottom-0" style={{
        animationDelay: '1s'
      }}></div>
      </div>
...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    const results = bubbles.filter(bubble => bubble.topic && bubble.topic.toLowerCase().includes(searchTermLower) || bubble.name && bubble.name.toLowerCase().includes(searchTermLower) || bubble.username && bubble.username.toLowerCase().includes(searchTermLower));
  ...
  useEffect(() => {
    const fetchBubbles = async () => {
      try {
        ...
        const {
          data: regularData,
          error: regularError
        } = await supabase.from('bubbles').select('*').order('created_at', {
          ascending: false
        });
        ...
        } = await supabase.from('bubbles').select('*').order('reflect_count', {
          ascending: false
        }).gt('reflect_count', 0).limit(5);
    ...
    const channel = supabase.channel('bubbles-dashboard').on('postgres_changes', {
      event: 'INSERT',
      schema: 'public',
      table: 'bubbles'
    }, payload => {
      ...
      setBubbles(currentBubbles => {
        ...
        setFilteredBubbles(prev => {
          ...
          return newBubbles.filter(bubble => bubble.topic && bubble.topic.toLowerCase().includes(searchTermLower) || bubble.name && bubble.name.toLowerCase().includes(searchTermLower) || bubble.username && bubble.username.toLowerCase().includes(searchTermLower));
        });
        return newBubbles;
      });
    }).on('postgres_changes', {
      ...
      setBubbles(currentBubbles => {
        const updatedBubbles = currentBubbles.map(bubble => bubble.id === updatedBubble.id ? updatedBubble : bubble);
        setFilteredBubbles(prev => {
          const searchTermLower = searchTerm.toLowerCase();
          if (!searchTerm) return updatedBubbles;
          return updatedBubbles.filter(bubble => bubble.topic && bubble.topic.toLowerCase().includes(searchTermLower) || bubble.name && bubble.name.toLowerCase().includes(searchTermLower) || bubble.username && bubble.username.toLowerCase().includes(searchTermLower));
        });
      ...
      setMostReflectedBubbles(currentBubbles => {
        ...
        if (exists) {
          // If exists, update it
          return currentBubbles.map(b => b.id === updatedBubble.id ? updatedBubble : b).sort((a, b) => (b.reflect_count || 0) - (a.reflect_count || 0));
        } else if ((updatedBubble.reflect_count || 0) > 0) {
          // If doesn't exist but has reflections, add it and sort
          return [...currentBubbles, updatedBubble].sort((a, b) => (b.reflect_count || 0) - (a.reflect_count || 0)).slice(0, 5);
        }
  ...
  return <div className="space-y-6">
      ...
      {!showFullView && <Card className="mt-8 border-[#F9C80E]/20">
          ...
          <CardContent>
            {filteredBubbles.length > 0 ? <div className="space-y-4">
                {filteredBubbles.map(bubble => <div key={bubble.id} className="p-4 border border-[#F9C80E]/20 rounded-lg hover:bg-[#FFFDF5] transition cursor-pointer" onClick={() => navigate(`/bubble/${bubble.id}`)}>
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <ReflectButton bubbleId={bubble.id} reflectCount={bubble.reflect_count || 0} />
                    </div>
                    <p className="text-sm font-medium">Topic: {bubble.topic}</p>
                    {bubble.description && <p className="text-sm text-gray-600">{bubble.description}</p>}
                    <div className="flex justify-between text-xs text-gray-500 mt-2">
                      <span>Reflections: {bubble.reflect_count || 0}</span>
                    </div>
                  </div>)}
              </div> : <EmptyBubbleState />}
          </CardContent>
        </Card>}
    </div>;
};
